...
<div class="col-lg-12 stretch-card">
    <div class="card">
      <div class="card-body">
        ...
        <a href="/masuk/lensa/kmccr/export_excel" class="btn btn-success my-3" target="_blank">EXPORT EXCEL</a>
        ...
        <div class="table-responsive">
          <table class="table table-bordered table-contextual table-hover table-striped">
            ...
            <tbody>
             ...
             @foreach ($lensa_kmccr_masuk as $masuk)

             <tr>
               <td> {{ $no++ }} </td>
               <td> {{ $masuk -> lensa_kmccr -> ukuran}} </td>
               <td> {{ $masuk -> updated_at}} </td>
               <td> {{ $masuk -> jumlah_masuk }} </td>
               <td> {{ $masuk -> pj }} </td>
               <td> 
                    <a href="/lensakmccrmasuk/{{$masuk->id}}/edit" class="btn btn-warning btn-sm" role="button">Edit</a>
                    <a href="/lensakmccrmasuk/delete/{{$masuk->id}}" class="btn btn-danger btn-sm" role="button">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/super/barangmasuk/lensa/kmccr/create" method="POST">
                        {{csrf_field()}}
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Ukuran Lensa</label>
                                <select name="lensa_kmccr_id" class="select2" multiple="multiple" id="exampleFormControlSelect1" style="width: 100%">
                                    @foreach ($lensa_kmccr_array as $array)
