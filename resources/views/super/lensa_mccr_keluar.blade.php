...
<div class="col-lg-12 stretch-card">
    <div class="card">
      <div class="card-body">
        ...
        <a href="/keluar/lensa/mccr/export_excel" class="btn btn-success my-3" target="_blank">EXPORT EXCEL</a>
        ...
        <div class="table-responsive">
          <table class="table table-bordered table-contextual table-hover table-striped">
            ...
            <tbody>
             ...
             @foreach ($lensa_mccr_keluar as $keluar)

             <tr>
               <td> {{ $no++ }} </td>
               <td> {{ $keluar -> lensa_mccr -> ukuran}} </td>
               <td> {{ $keluar -> updated_at}} </td>
               <td> {{ $keluar -> jumlah_keluar }} </td>
               <td> {{ $keluar -> pj }} </td>
               <td> {{ $keluar -> tujuan -> toko }} </td>
               <td>
                    <a href="/lensamccrkeluar/{{$keluar->id}}/edit" class="btn btn-warning btn-sm" role="button">Edit</a>
                    <a href="/lensamccrkeluar/delete/{{$keluar->id}}" class="btn btn-danger btn-sm" role="button">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/super/barangkeluar/lensa/mccr/create" method="POST">
                        {{csrf_field()}}
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Ukuran Lensa</label>
                                <select name="lensa_mccr_id" class="select2" multiple="multiple" id="exampleFormControlSelect1" style="width: 100%">
                                    @foreach ($lensa_mccr_array as $array)
