...
<div class="col-lg-12 stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Lensa MCCR Keluar</h4>
        <a href="/keluar/lensa/mccrkaca/export_excel" class="btn btn-success my-3" target="_blank">EXPORT EXCEL</a>
        ...
        <div class="table-responsive">
          <table class="table table-bordered table-contextual table-hover table-striped">
            ...
            <tbody>
             ...
             @foreach ($lensa_mccrkaca_keluar as $keluar)

             <tr>
               <td> {{ $no++ }} </td>
               <td> {{ $keluar -> lensa_mccrkaca -> ukuran}} </td>
               <td> {{ $keluar -> updated_at}} </td>
               <td> {{ $keluar -> jumlah_keluar }} </td>
               <td> {{ $keluar -> pj }} </td>
               <td> {{ $keluar -> tujuan -> toko }} </td>
               <td>
                    <a href="/lensamccrkacakeluar/{{$keluar->id}}/edit" class="btn btn-warning btn-sm" role="button">Edit</a>
                    <a href="/lensamccrkacakeluar/delete/{{$keluar->id}}" class="btn btn-danger btn-sm" role="button">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/super/barangkeluar/lensa/mccr/kaca/create" method="POST">
                        {{csrf_field()}}
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">Ukuran Lensa</label>
                                <select name="lensa_mccrkaca_id" class="select2" multiple="multiple" id="exampleFormControlSelect1" style="width: 100%">
                                    @foreach ($lensa_mccrkaca_array as $array)
