...
<div class="col-lg-12 stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Idol Black Masuk</h4>
        <a href="/masuk/softlens/idolblack/export_excel" class="btn btn-success my-3" target="_blank">EXPORT EXCEL</a>
        ...
        <div class="table-responsive">
          <table class="table table-bordered table-contextual table-hover table-striped">
            ...
            <tbody>
             ...
             @foreach ($idol_black_masuk as $masuk)

             <tr>
               <td> {{ $no++ }} </td>
               <td> {{ $masuk -> idol_black -> ukuran}} </td>
               <td> {{ $masuk -> updated_at}} </td>
               <td> {{ $masuk -> jumlah_masuk }} </td>
               <td> {{ $masuk -> pj }} </td>
               <td> 
                    <a href="/idolblackmasuk/{{$masuk->id}}/edit" class="btn btn-warning btn-sm" role="button">Edit</a>
                    <a href="/idolblackmasuk/delete/{{$masuk->id}}" class="btn btn-danger btn-sm" role="button">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="/super/barangmasuk/softlens/idolblack/create" method="POST">
                        {{csrf_field()}}
